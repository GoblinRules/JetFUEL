...
- If Tailscale says `failed to connect to local tailscaled`, the daemon did not start or its socket was not ready. `Repair Tailscale` writes a boot hook that waits for networking and `/dev/net/tun` before starting `tailscaled`, then keeps a small watchdog running to restart it if it later exits.
